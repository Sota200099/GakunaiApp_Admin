            ...
            <?php foreach($logs as $value):?>
                <form action="delete_post.php" method="post">
                    <?php if($value["delete_flag"]==0):?>
                        <p class="mb-4"><?=$value["title"]?>　投稿者：<?=$value["name"]?>　<?=$value["date"]?><button type="submit" style="position:fixed;right:10%;" class="<?=$value["title"]?> btn btn-outline-primary" value="<?=$value["title"]?>" onclick="input_confirm()">投稿を削除</button></p>
                    <?php endif?>
